...
import joblib
from pydantic import BaseModel, Field #data validation and parsing.
from surprise import Dataset, Reader #r building and analyzing recommender systems.
import pandas as pd
...
# Load the data
data = pd.read_csv("DATA1.csv", usecols=['UserId', 'ProductName', 'Rating'])

# Prepare the dataset for the surprise library
reader = Reader(rating_scale=(data['Rating'].min(), data['Rating'].max()))# Defines the rating scale based on the data.
dataset = Dataset.load_from_df(data[['UserId', 'ProductName', 'Rating']], reader)#Loads the dataset in a format suitable for the surprise
trainset = dataset.build_full_trainset()#Builds a training set from the dataset.
...
class Item(BaseModel):
    ...
    Rating: int = Field(..., gt=0,le=5, description="Rating must be greater than 0")
...
def get_recommendations(item: Item):
    ...
    try:
        ...
        dataset = Dataset.load_from_df(data[['UserId', 'ProductName', 'Rating']], reader)
